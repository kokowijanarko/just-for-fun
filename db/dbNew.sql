...
CREATE TABLE `dev_user` (
  ...
  `datetime_insert` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'date and time insert',
  PRIMARY KEY (`id`),
  UNIQUE KEY `level` (`level`),
  CONSTRAINT `dev_user_ibfk_1` FOREIGN KEY (`level`) REFERENCES `dev_level` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=latin1;

/*Data for the table `dev_user` */

insert  into `dev_user`(`id`,`name`,`username`,`password`,`level`,`description`,`datetime_insert`) values (1,'admin','admin','21232f297a57a5a743894a0e4a801fc3',1,'root\r\n','2016-01-28 21:58:50');
...
CREATE TABLE `inv_history` (
  ...
  CONSTRAINT `inv_history_ibfk_2` FOREIGN KEY (`history_insert_user_id`) REFERENCES `dev_user` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=latin1;

/*Data for the table `inv_history` */

insert  into `inv_history`(`history_id`,`history_inv_id`,`history_condition_id`,`history_desc`,`history_insert_user_id`,`history_insert_timestamp`) values (1,1,2,'cat mengelupas',1,'2016-05-29 10:02:33');
